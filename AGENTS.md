...
- For every task that changes source code or project configuration, use the configured SonarQube MCP to inspect both service projects before remediation and again after changes. Confirm each SonarQube project, branch, and analyzed revision; do not assume a previous or partial analysis represents the current checkout.
- The SonarQube Community Build edition used by this project analyzes its default/main branch only and rejects `sonar.branch.name`. For that edition, omit the branch parameter and explicitly report the default branch; use branch analysis only when the configured edition supports it. Run the scanner separately from the MCP; the MCP is for inspecting the resulting analysis and findings.
- Use `scripts/sonar-scan.sh` as the canonical local scan command. It runs Rust unit/integration and frontend unit tests with coverage and submits separate analyses from `sonar-project-api.properties` and `sonar-project-frontend.properties`; run Playwright E2E separately against the deployed frontend. Provide project-scoped `SONAR_API_TOKEN` and `SONAR_FRONTEND_TOKEN` plus, when needed, `SONAR_HOST_URL` or `SONAR_URL` through the environment without printing or committing secrets.
